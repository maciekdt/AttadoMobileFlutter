...
class TasksCustomFilteringForm extends StatefulWidget {
  const TasksCustomFilteringForm({super.key});

  @override
  State<StatefulWidget> createState() => _TasksCustomFilteringFormState();
}

class _TasksCustomFilteringFormState extends State<TasksCustomFilteringForm> {
  late TasksOptionsProvider _optionsProvider;

  void _selectMyFilter() async {
    var result = await context.push("/tasks/filter/my-filter");
    if (result != null) {
  ...
  void _selectSystemFilter() async {
    var result = await context.push("/tasks/filter/system-filter");
    if (result != null) {
